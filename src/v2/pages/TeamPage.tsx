...
import { CheckCircle2, Circle, Share2, Target, ClipboardList, RefreshCw, Users } from 'lucide-solid';
  ...
  return (
    ...
    <Show when={(!goalsList.loading || !!goalsList.latest) && (!storiesList.loading || !!storiesList.latest)} fallback={<TeamSkeleton />}>
      <div class="space-y-4">
        ...
        <Show when={sharedGoals().length > 0 || sharedStories().length > 0}>
          <details class="group rounded-2xl border border-base-content/[0.045] bg-base-100/30">
            <summary class="flex h-12 cursor-pointer list-none items-center gap-2 px-3 text-left [&::-webkit-details-marker]:hidden">
              <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-base-content/[0.04]">
                <Share2 size={13} strokeWidth={2.4} class="text-base-content/45" />
              </span>
              <span class="min-w-0 flex-1 truncate text-[11px] font-bold uppercase tracking-[0.12em] text-base-content/38">
                Contexto compartido
              </span>
              <Show when={sharedGoals().length > 0}>
                <span class="rounded-full bg-base-content/[0.04] px-2 py-1 text-[10px] font-bold text-base-content/42 tabular-nums">
                  {sharedGoals().length} obj.
                </span>
              </Show>
              <Show when={sharedStories().length > 0}>
                <span class="rounded-full bg-ios-blue-500/10 px-2 py-1 text-[10px] font-bold text-ios-blue-500/78 tabular-nums">
                  {sharedStories().length} HUs
                </span>
              </Show>
            </summary>

            <div class="flex flex-wrap gap-3 border-t border-base-content/[0.04] p-3">
              <Show when={sharedGoals().length > 0}>
                <section class="min-w-0 flex-1 basis-[360px] space-y-2 rounded-2xl border border-base-content/[0.045] bg-base-100/36 p-3">
                  <div class="flex items-center gap-2">
                    <div class="flex h-7 w-7 items-center justify-center rounded-xl bg-base-content/[0.04]">
                  ...
                  <div class="grid grid-cols-1 gap-2 xl:grid-cols-2">
                    <For each={sharedGoals()}>
                      {(goal) => {
                        ...
                        return (
                          <div class="group flex items-start gap-3 rounded-xl border border-base-content/[0.045] bg-base-100/42 px-3 py-2.5 transition-colors hover:bg-base-content/[0.016]">
                  ...
                  <div class="grid grid-cols-1 gap-2 xl:grid-cols-2">
                    <For each={sharedStories()}>
                      {(story) => {
                        ...
                        return (
                          <button type="button" onClick={() => setSelectedStory(story as Story)} class="group flex w-full cursor-pointer items-center gap-3 rounded-xl border border-base-content/[0.045] bg-base-100/42 px-3 py-2.5 text-left transition-colors hover:bg-base-content/[0.016] hover:border-base-content/[0.10]">
